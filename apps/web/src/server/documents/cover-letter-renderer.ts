...
export function renderDesignedCoverLetterHtml(resume: StructuredResume, letter: string): string {
  ...
  </style></head><body><main class="sheet"><header><div><h1>${escape(resume.identity.fullName)}</h1><p>${escape(resume.targetTitle)}</p></div><div class="contact">${escape(resume.identity.email)}<br>${escape(formatUsPhone(resume.identity.phone))}</div></header>
